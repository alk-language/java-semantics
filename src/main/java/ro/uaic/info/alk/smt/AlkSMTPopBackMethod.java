...
implements SMTMethodSolver
{
    ...
    public Expr<?> apply(Expr<?> target, Expr<?>[] params)
    {
        ...
        {
            throw new AlkException("Applying popBack built-in method on non-array expression!");
        }
    ...
    private Expr doConstraints(Sort arrayType, Expr application, Expr target, Expr[] params)
    {
        ...
        Expr body1 = support.applyEqPrefix(application, target, sizeFnc.apply(application, null));
        Expr body2 = ctx.mkEq(left.apply(application), left.apply(target));
        Expr body3 = ctx.mkEq(right.apply(application), ctx.mkSub(right.apply(target), ctx.mkInt(1)));
    ...
    private FuncDecl<?> getFunc(Sort arrayType)
    {
        ...
        {
            FuncDecl fd = alkCtx.ctx.mkFuncDecl("popBack", arrayType, arrayType);
