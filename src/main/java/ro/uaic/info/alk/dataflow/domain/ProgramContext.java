...
import ro.uaic.info.alk.util.Recurrence;
...
class ExecutionPath
{
    ...
    public ExecutionPath(ExecutionPath path)
    {
        ...
        this.env = path.env.clone(locMapping, store);
    ...
    public void run(AST tree, EdgeData data)
    {
        ExecutionInterpreter manager =
                new ExecutionInterpreter(new SymbolicStatefulExpressionInterpreter(), new SymbolicStatefulStmtInterpreter());
        Storable value = Stepper.run(tree, new AlkConfiguration(), env, store, this.pc, new FuncManager(), manager);
        if (data != null)
        {
